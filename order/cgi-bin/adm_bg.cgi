...
#   @new_bg      - products which are new in this Bijenpark Geuzenveld list
#   @bg_maybe    - Bijenpark Geuzenveld products not currently on offer, but maybe later
#   @bg_all      - all Bijenpark Geuzenveld products which are not vokomokum products
...
sub get_stats {
    ...
    my $h;
    my $a;
    my (@voko_changed, @voko_dropped, @voko_same, @new_bg, 
	@bg_maybe, @bg_all);

    if($config->{choice} == 0) {
	#$sth = prepare("UPDATE bg_data SET is_seen = 'f'", $dbh);
	#$sth->execute;
	#$sth->finish;
    }
    # make sure the is_product flag are correctly set
    $sth = prepare("UPDATE bg_data SET is_product = (wh_prcode IN " .
    ...
    $sth = prepare("SELECT * FROM bg_interval()", $dbh);
    $sth->execute;
    ...
    $sth = prepare("SELECT * from bg_products ORDER BY wh_pr_id", $dbh);
    $sth->execute;
    while($h = $sth->fetchrow_hashref) {
	$h->{reprice} = 0;
	my $wid = $h->{wh_pr_id};
	...
	if(defined($h->{pr_id})) {
		...
		push @bg_maybe, $h if(not $h->{is_skipped});
		push @bg_all, $h if($h->{is_skipped});
		next;
	    }
...
sub get_vars {
	...
	foreach $wid (@updates) {
	    ...
	    my $bgst = prepare("SELECT wh_wh_q, wh_btw, wh_whpri, ".
				"wh_prcode, wh_descr FROM bg_data ".
				"WHERE wh_pr_id = ?", $dbh);
	    $bgst->execute($wid);
	    my $h = $bgst->fetchrow_hashref;
	    $bgst->finish;
    ...
    my $do_commit = 0;
    my $upd_sth = prepare("UPDATE bg_data SET is_seen = ?, ".
...
sub do_dropped_products {
    ...
    my $tpl = new CGI::FastTemplate($config->{templates});
    $tpl->strict();
    $tpl->define( header  => "adm_bg/adm_bg_m1_dropped.template",
	);
    $tpl->assign({});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
	...
	$tpr->strict();
	$tpr->define( row => "adm_bg/adm_bg_m1_dropped_row.template");
	$tpr->assign($h);
	$tpr->parse(MAIN => "row");
	$tpr->print("MAIN");
    }
...
sub title_row {
    ...
    $tpr->define( title => ($config->{choice} & 8) ?
		  "adm_bg/adm_bg_m101112_table.template" : 
		  "adm_bg/adm_bg_m234_table.template");
    $tpr->assign({});
...
sub do_changed_products {
    ...
    my $tpl = new CGI::FastTemplate($config->{templates});
    $tpl->strict();
    $tpl->define( header  => "adm_bg/adm_bg_m1_changed.template",
	);
    $tpl->assign({});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_bg/adm_bg_m1_changed_row.template",
		      url => (($h->{wh_url}) ? "common/bg_url.template" : "common/no_url.template"));
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
	$tpr->print("MAIN");
...
sub mode_1 {
    ...
    html_header("Product File Changes from Bijenpark Geuzenveld Product List",
		"adm_bg/adm_bg_mode_1.template", $config, $cgi, $dbh);
...
sub mode_2 {
    my ($new_vals, $new_bg, $config, $cgi, $dbh) = @_;

    do_header_234("adm_bg/adm_bg_m2_title.template", $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $new_bg, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $new_bg, $config, $cgi, $dbh);
}

sub mode_3 {
    my ($new_vals, $bg_maybe, $config, $cgi, $dbh) = @_;

    do_header_234("adm_bg/adm_bg_m3_title.template", $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $bg_maybe, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $bg_maybe, $config, $cgi, $dbh);
}

sub mode_4 {
    my ($new_vals, $bg_all, $config, $cgi, $dbh) = @_;

    do_header_234("adm_bg/adm_bg_m4_title.template", $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $bg_all, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $bg_all, $config, $cgi, $dbh);
}
...
sub do_footer_234 {
    ...
    $tpf->strict();
    $tpf->define( footer => "adm_bg/adm_bg_m234_footer.template");
...
sub modes_234 {
    my ($new_vals, $bg_hash, $config, $cgi, $dbh) = @_;
    my $sth = prepare("SELECT min_price(?, ?, ?, ?)", $dbh);
    my $line = 0;

    foreach my $h (@{$bg_hash}) {
	my $wid = $h->{wh_pr_id};
	...
	$tpr->strict();
	$tpr->define( row => "adm_bg/adm_bg_m2_row.template",
		      url => (($h->{wh_url}) ? "common/bg_url.template" : "common/no_url.template"));
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
...
sub modes_101112 {
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_bg/adm_bg_m101112_row.template",
		      url => (($h->{wh_url}) ? "common/bg_url.template" : "common/no_url.template"));
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
...
sub doit {
    ...
    get_choice($voko_changed, $voko_dropped, $voko_same, 
	       $new_bg, $bg_maybe, $bg_all, $config, $cgi, $dbh) 
	if(not $config->{choice});
    mode_1($voko_changed, $voko_dropped, $config, $cgi, $dbh)
	if($config->{choice} == 1);
    mode_2($new_vals, $new_bg, $config, $cgi, $dbh) 
	if(($config->{choice} & ~8) == 2);
    mode_3($new_vals, $bg_maybe, $config, $cgi, $dbh) 
	if(($config->{choice} & ~8) == 3);
    mode_4($new_vals, $bg_all, $config, $cgi, $dbh) 
