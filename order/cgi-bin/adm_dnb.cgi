...
#   @new_dnb      - products which are new in this dnb list
#   @dnb_maybe    - dnb products not currently on offer, but maybe later
#   @dnb_all      - all dnb products which are not vokomokum products
# each array element is a row from dnb_products view
...
sub get_stats {
    ...
    my $h;
    my $a;
    my (@voko_changed, @voko_dropped, @voko_same, @new_dnb, 
	@dnb_maybe, @dnb_all);

      if($config->{choice} == 0) {
    # 	$sth = prepare("UPDATE dnbdata SET is_seen = 'f'", $dbh);
    # 	$sth->execute;
    # 	$sth->finish;
    }
    # make sure the is_product flag are correctly set
    $sth = prepare("UPDATE dnbdata SET is_product = (wh_prcode IN " .
    ...
    $sth = prepare("SELECT * FROM dnb_interval()", $dbh);
    $sth->execute;
    ...
    $sth = prepare("SELECT * from dnb_products ORDER BY wh_pr_id", $dbh);
    $sth->execute;
...
sub get_vars {
	...
	foreach $wid (@updates) {
	    ...
	    my $dnbst = prepare("SELECT wh_wh_q, wh_btw, wh_whpri, ".
				"wh_prcode, wh_descr FROM dnbdata ".
				"WHERE wh_pr_id = ?", $dbh);
	    $dnbst->execute($wid);
    ...
    my $do_commit = 0;
    my $upd_sth = prepare("UPDATE dnbdata SET is_seen = ?, ".
...
sub do_dropped_products {
    ...
    my $tpl = new CGI::FastTemplate($config->{templates});
    $tpl->strict();
    $tpl->define( header  => "adm_dnb/adm_dnb_m1_dropped.template",
	);
    $tpl->assign({});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
	...
	$tpr->strict();
	$tpr->define( row => "adm_dnb/adm_dnb_m1_dropped_row.template");
	$tpr->assign($h);
	$tpr->parse(MAIN => "row");
	$tpr->print("MAIN");
    }
...
sub title_row {
    ...
    $tpr->define( title => ($config->{choice} & 8) ?
		  "adm_dnb/adm_dnb_m101112_table.template" : 
		  "adm_dnb/adm_dnb_m234_table.template");
    $tpr->assign({});
...
sub do_changed_products {
    ...
    my $tpl = new CGI::FastTemplate($config->{templates});
    $tpl->strict();
    $tpl->define( header  => "adm_dnb/adm_dnb_m1_changed.template",
	);
    $tpl->assign({});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_dnb/adm_dnb_m1_changed_row.template",
...
sub mode_1 {
    ...
    html_header("Product File Changes from DNB Product List",
...
sub mode_2 {
    ...
    modes_234($new_vals, $new_dnb, $config, $cgi, $dbh);
}
...
sub mode_3 {
    ...
    modes_234($new_vals, $dnb_maybe, $config, $cgi, $dbh);
}
...
sub mode_4 {
    ...
    modes_234($new_vals, $dnb_all, $config, $cgi, $dbh);
}
...
sub do_header_234 {
    ...
    $tpl->assign({ mode => $config->{choice},
		   toggle => ($config->{choice} & 8) ?
		       "Return to DNB list" :  "Edit New Products"});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
}
...
sub do_footer_234 {
    ...
    $tpf->strict();
    $tpf->define( footer => "adm_dnb/adm_dnb_m234_footer.template");
...
sub modes_234 {
    my ($new_vals, $dnb_hash, $config, $cgi, $dbh) = @_;
    my $sth = prepare("SELECT min_price(?, ?, ?, ?)", $dbh);
    my $line = 0;

    foreach my $h (@{$dnb_hash}) {
	my $wid = $h->{wh_pr_id};
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_dnb/adm_dnb_m2_row.template",
		      url => "common/dnb_url.template");
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
...
sub modes_101112 {
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_dnb/adm_dnb_m101112_row.template",
		      url => "common/dnb_url.template");
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
...
sub mode_5 {
    ...
    $tpl->define( header         => "common/header.template",
                  banner         => "common/adm-banner.template",
		  buttons        => "adm_dnb/adm_dnb_file_upload.template",
	);
    my %hdr_h =(  Pagename       => 'Upload DNB File',
		  Title          => 'Upload DNB File',
		  Nextcgi        => 'adm_do_dnb_upload.cgi',
		  mem_name       => $config->{mem_name},
	);
...
sub doit {
    ...
    get_choice($voko_changed, $voko_dropped, $voko_same, 
	       $new_dnb, $dnb_maybe, $dnb_all, $config, $cgi, $dbh) 
	if(not $config->{choice});
    mode_1($voko_changed, $voko_dropped, $config, $cgi, $dbh)
	if($config->{choice} == 1);
    mode_2($new_vals, $new_dnb, $config, $cgi, $dbh) 
	if(($config->{choice} & ~8) == 2);
    mode_3($new_vals, $dnb_maybe, $config, $cgi, $dbh) 
	if(($config->{choice} & ~8) == 3);
    mode_4($new_vals, $dnb_all, $config, $cgi, $dbh) 
...
sub main {
    ...
    $config->{nextcgi}  = "adm_dnb.cgi";
    $config->{title}    = "Process DNB product List";
    $config->{choice} = 0;
