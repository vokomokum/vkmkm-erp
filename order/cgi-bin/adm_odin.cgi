...
#   @new_odin      - products which are new in this odin list
#   @odin_maybe    - odin products not currently on offer, but maybe later
#   @odin_all      - all odin products which are not vokomokum products
# each array element is a row from odin_products view
...
sub get_stats {
    ...
    $sth = prepare("SELECT * FROM odin_interval()", $dbh);
    $sth->execute;
	...
	if($h->{is_product}) {
	    ...
	    # it's in the voko product list, but no longer a odin product
	    if($h->{wh_last_seen} lt $newest) {
		push @voko_dropped, $h;
		next;
	    }
	...
	# odin product not in the voko list. Skip if no longer a odin product
...
#              new odin products
#              all non-excluded odin products
#              all odin products
...
sub get_choice {
    ...
    $tpl->define( header         => "common/header.template",
		  ...
		  buttons        => "adm_odin/odin-prod-type.template");
...
# the has will be merged into the wholesal records hash
sub get_vars {
    my ($config, $cgi, $dbh) = @_;
    ...
    # handle any reclassifications now
    my $do_commit = 0;
    my $upd_sth = prepare("UPDATE odindata SET is_seen = ?, ".
...
sub do_dropped_products {
	...
	$tpr->strict();
	$tpr->define( row => "adm_odin/adm_odin_m1_dropped_row.template");
	$tpr->assign($h);
	$tpr->parse(MAIN => "row");
	$tpr->print("MAIN");
    }
...
sub do_changed_products {
    ...
    my $tpl = new CGI::FastTemplate($config->{templates});
    $tpl->strict();
    $tpl->define( header  => "adm_odin/adm_odin_m1_changed.template",);
    $tpl->assign({});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_odin/adm_odin_m1_changed_row.template",
		      url => "common/odin_url.template");
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
	$tpr->print("MAIN");
...
#
# 3) products where the price is below the margin for whatever reason (shouldn't 
#     happen) or price > what the margin requires (adjust price up or margin down)
sub mode_1 {
    my ($voko_changed, $voko_dropped, $config, $cgi, $dbh) = @_;

    html_header("Product File Changes from Odin Product List",
		"adm_odin/adm_odin_mode_1.template", $config, $cgi, $dbh);
...
# mode 2 - new Odin products
# admin can make it a product while setting the description, category
# price, and margin, can mark it as a Maybe or a Never

sub mode_2 {
    my ($new_vals, $new_odin, $config, $cgi, $dbh) = @_;

    do_header_234("adm_odin/adm_odin_m2_title.template", 
		  "New Products in the  Odin Product List",
		  $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $new_odin, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $new_odin, $config, $cgi, $dbh);
}

sub mode_3 {
    my ($new_vals, $odin_maybe, $config, $cgi, $dbh) = @_;

    do_header_234("adm_odin/adm_odin_m3_title.template", 
		  "Possibly Interesting Products in the OdinProduct List",
		  $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $odin_maybe, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $odin_maybe, $config, $cgi, $dbh);
}

sub mode_4 {
    my ($new_vals, $odin_all, $config, $cgi, $dbh) = @_;

    do_header_234("adm_odin/adm_odin_m4_title.template", 
		  "All Products in the Odin Product List",
		  $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $odin_all, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $odin_all, $config, $cgi, $dbh);
}
...
sub do_header_234 {
    ...
    $tpl->assign({ mode => $config->{choice},
		   toggle => ($config->{choice} & 8) ?
		       "Return to Odin list" :  "Edit New Products"});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
}
...
sub do_footer_234 {
    ...
    $tpf->strict();
    $tpf->define( footer => "adm_odin/adm_odin_m234_footer.template");
...
sub modes_234 {
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_odin/adm_odin_m2_row.template",
		      url => "common/odin_url.template");
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
...
sub modes_101112 {
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_odin/adm_odin_m101112_row.template",
		      url => "common/odin_url.template");
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
...
sub mode_5 {
    ...
    $tpl->define( header         => "common/header.template",
                  banner         => "common/adm-banner.template",
		  buttons        => "adm_odin/adm_odin_file_upload.template",
	);
    my %hdr_h =(  Pagename       => 'Upload Odin File',
		  Title          => 'Upload Odin File',
		  Nextcgi        => 'adm_do_odin_upload.cgi',
		  mem_name       => $config->{mem_name},
	);
...
sub doit {
	...
	if($config->{choice} == 1);
    mode_2($new_vals, $new_odin, $config, $cgi, $dbh) 
	if(($config->{choice} & ~8) == 2);
    mode_3($new_vals, $odin_maybe, $config, $cgi, $dbh) 
	if(($config->{choice} & ~8) == 3);
    mode_4($new_vals, $odin_all, $config, $cgi, $dbh) 
...
sub main {
    ...
    $config->{nextcgi}  = "adm_odin.cgi";
    $config->{title}    = "Process Odin product List";
    $config->{choice} = 0;
