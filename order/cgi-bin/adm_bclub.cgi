...
my $workbook;
my $worksheet;
my $fname = "bclub_data/bclub_data.xls";
...
sub open_spreadsheet{
  ...
  if($dbh->err) {
    ...
    $worksheet->write('F8', "Bubble Club - $cutoff", $fmt_lck);
    $cutoff = $h->{wh_update};

    my @headings = ('Pr code', 'Description', 'Qty', 'Price', 'BTW%', 'URL'); 
    $worksheet->write_row('A9', \@headings, $fmt_lck);
    my $row = 10;
    $sth = prepare('SELECT * FROM bclub_data WHERE wh_last_seen = ? ORDER BY wh_pr_id' , $dbh);
    $sth->execute($cutoff);
...
sub get_stats {
    ...
    my $h;
    my $a;
    my (@voko_changed, @voko_dropped, @voko_same, @new_bclub, 
	@bclub_maybe, @bclub_all);

    if($config->{choice} == 0) {
	#$sth = prepare("UPDATE bclub_data SET is_seen = 'f'", $dbh);
	#$sth->execute;
	#$sth->finish;
    }
    # make sure the is_product flag are correctly set
    $sth = prepare("UPDATE bclub_data SET is_product = (wh_prcode IN " .
    ...
    $sth = prepare("SELECT * FROM bclub_interval()", $dbh);
    $sth->execute;
    ...
    $sth = prepare("SELECT * from bclub_products ORDER BY wh_pr_id", $dbh);
    $sth->execute;
    while($h = $sth->fetchrow_hashref) {
	$h->{reprice} = 0;
	my $wid = $h->{wh_pr_id};
	...
	if(defined($h->{pr_id})) {
		...
		push @bclub_maybe, $h if(not $h->{is_skipped});
    ...
    return (\@voko_changed, \@voko_dropped, \@voko_same, 
	    \@new_bclub, \@bclub_maybe, \@bclub_all);}
...
sub get_choice {
    my ($voko_changed, $voko_dropped, $voko_same, 
	    $new_bclub, $bclub_maybe, $bclub_all, $config, $cgi, $dbh) = @_;
    ...
    my %hdr_h =(  Pagename       => $config->{title},
		  ...
		  new            => scalar(@{$new_bclub}),
		  maybe          => scalar(@{$bclub_maybe}),
		  all            => scalar(@{$bclub_all}),
		  filename       => "/$fname",
	);
...
sub get_vars {
	...
	foreach $wid (@updates) {
	    ...
	    my $bclubst = prepare("SELECT wh_wh_q, wh_btw, wh_whpri, ".
				"wh_prcode, wh_descr FROM bclub_data ".
				"WHERE wh_pr_id = ?", $dbh);
	    $bclubst->execute($wid);
	    my $h = $bclubst->fetchrow_hashref;
	    $bclubst->finish;
    ...
    my $do_commit = 0;
    my $upd_sth = prepare("UPDATE bclub_data SET is_seen = ?, ".
	...
	foreach $wid (@updates) {
	    ...
	    $ins_sth->execute($inp->{c}, 99999, 
			      $config->{BUBBLE_CLUB}->{bc_wh_id}, $h->{wh_wh_q}, 
			      $inp->{m}, $inp->{q}, 
...
sub do_dropped_products {
    ...
    my $tpl = new CGI::FastTemplate($config->{templates});
    $tpl->strict();
    $tpl->define( header  => "adm_bclub/adm_bclub_m1_dropped.template",
	);
    $tpl->assign({});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
	...
	$tpr->strict();
	$tpr->define( row => "adm_bclub/adm_bclub_m1_dropped_row.template");
	$tpr->assign($h);
	$tpr->parse(MAIN => "row");
	$tpr->print("MAIN");
    }
...
sub title_row {
    ...
    $tpr->define( title => ($config->{choice} & 8) ?
		  "adm_bclub/adm_bclub_m101112_table.template" : 
		  "adm_bclub/adm_bclub_m234_table.template");
    $tpr->assign({});
...
sub do_changed_products {
    ...
    my $tpl = new CGI::FastTemplate($config->{templates});
    $tpl->strict();
    $tpl->define( header  => "adm_bclub/adm_bclub_m1_changed.template",
	);
    $tpl->assign({});
    $tpl->parse(MAIN => "header");
    $tpl->print("MAIN");
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_bclub/adm_bclub_m1_changed_row.template",
...
sub mode_1 {
    ...
    html_header("Product File Changes from Bijenpark Geuzenveld Product List",
		"adm_bclub/adm_bclub_mode_1.template", $config, $cgi, $dbh);
...
sub mode_2 {
    ...
    do_header_234("adm_bclub/adm_bclub_m2_title.template", $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $new_bclub, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $new_bclub, $config, $cgi, $dbh);
}

sub mode_3 {
    my ($new_vals, $bclub_maybe, $config, $cgi, $dbh) = @_;

    do_header_234("adm_bclub/adm_bclub_m3_title.template", $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $bclub_maybe, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $bclub_maybe, $config, $cgi, $dbh);
}

sub mode_4 {
    my ($new_vals, $bclub_all, $config, $cgi, $dbh) = @_;

    do_header_234("adm_bclub/adm_bclub_m4_title.template", $config, $cgi, $dbh);
    if($config->{choice} & 8) {
	modes_101112($new_vals, $bclub_all, $config, $cgi, $dbh);
    }
    modes_234($new_vals, $bclub_all, $config, $cgi, $dbh);
}
...
sub do_footer_234 {
    ...
    $tpf->strict();
    $tpf->define( footer => "adm_bclub/adm_bclub_m234_footer.template");
...
sub modes_234 {
	...
	$tpr->strict();
	$tpr->define( row => "adm_bclub/adm_bclub_m2_row.template",
		      url => (($h->{wh_url}) ? "common/bclub_url.template" : "common/no_url.template"));
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
...
sub modes_101112 {
	...
	my $tpr = new CGI::FastTemplate($config->{templates});
	$tpr->strict();
	$tpr->define( row => "adm_bclub/adm_bclub_m101112_row.template",
		      url => (($h->{wh_url}) ? "common/bclub_url.template" : "common/no_url.template"));
	$tpr->assign($h);
	$tpr->parse("URL", "url");
	$tpr->parse(MAIN => "row");
...
sub mode_5 {
    ...
    $tpl->define( header         => "common/header.template",
                  banner         => "common/adm-banner.template",
		  buttons        => "adm_bclub/adm_bclub_file_upload.template",
	);
...
sub doit {
    ...
    get_choice($voko_changed, $voko_dropped, $voko_same, 
	       $new_bclub, $bclub_maybe, $bclub_all, $config, $cgi, $dbh) 
	if(not $config->{choice});
    mode_1($voko_changed, $voko_dropped, $config, $cgi, $dbh)
	if($config->{choice} == 1);
    mode_2($new_vals, $new_bclub, $config, $cgi, $dbh) 
	if(($config->{choice} & ~8) == 2);
    mode_3($new_vals, $bclub_maybe, $config, $cgi, $dbh) 
	if(($config->{choice} & ~8) == 3);
    mode_4($new_vals, $bclub_all, $config, $cgi, $dbh) 
